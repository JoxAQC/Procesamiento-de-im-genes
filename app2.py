...
# Interfaz de usuario
uploaded_files = st.file_uploader("Cargar imágenes (hasta 6)", 
                                type=['jpg', 'jpeg', 'png'], 
                                accept_multiple_files=True)

if uploaded_files and len(uploaded_files) > 0:
    # Limitar a 6 imágenes máximo
    if len(uploaded_files) > 6:
        st.warning("⚠️ Se han seleccionado más de 6 imágenes. Solo se procesarán las primeras 6.")
    ...
    for i, col in enumerate(cols):
        with col:
            img = load_image(uploaded_files[i])
            st.image(img, caption=f"Imagen {i+1}", use_container_width=True)
    
    # Seleccionar imagen para procesar
    selected_img_idx = st.selectbox("Seleccione la imagen a procesar", 
                                  range(len(uploaded_files)), 
                                  format_func=lambda x: f"Imagen {x+1}")
    
    # Cargar la imagen seleccionada
    img = load_image(uploaded_files[selected_img_idx])
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.image(img, caption=f"Imagen {selected_img_idx+1} - Original", use_container_width=True)
    ...
    if st.button("💾 Guardar imagen procesada"):
        ...
        if processed_img is not None:
            ...
            save_name = f"imagen_procesada_{selected_img_idx+1}.png"
            img_pil.save(save_name)
            st.success(f"✅ Imagen {selected_img_idx+1} guardada como '{save_name}'")
            with open(save_name, "rb") as file:
                st.download_button(
                    label=f"⬇️ Descargar imagen {selected_img_idx+1}",
                    data=file,
                    file_name=save_name,
                    mime="image/png"
                )
    ...
    st.info("ℹ️ Por favor, suba una o más imágenes (hasta 6) para comenzar el procesamiento")